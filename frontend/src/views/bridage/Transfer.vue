    ...
    <CommTipDialog ref="TipPopupRef">
      <div slot="PoperContent" class="dialog">
        <div class="dialog-box">
          ...
          <div class="title">
            Unlock more LayerX Bridge identities?
          </div>
          <div class="content">
            Explore more transactions on LayerX Bridge Finance Mainnet Have fun!
          </div>
          <div class="bottom">
  ...
  methods: {
    async openApiFilter() {
      ...
      refreshGasFeeToolTip() {
          ...
          const tradingFee = ` <br /><b>Fees using LayerX Bridge costs:</b><br />Trading Fee: $${ (
    ...
    async sendTransfer() {
      ...
      if (!walletIsLogin.value) {
        ...
        if (nonce > 8999) {
          this.$notify.error({
            title: `Address with the nonce over 9000 are not supported by LayerX Bridge`,
            duration: 3000,
          });
          return;
        }
        ...
        if (
          ...
          // TAG: prod test
          // this.$notify.error({
          //   title: `LayerX Bridge can only support minimum of ${ this.userMinPrice } and maximum of ${ this.maxPrice } ${ fromCurrency } on transfers.`,
          //   duration: 3000,
          // });
          // return;
        }
...
.transfer-box {
  ...
  .btn {
    ...
    line-height: 34px;
    margin-bottom: 20px;
    background: linear-gradient(90.46deg, #ff7b00 4.07%, #b7820a 98.55%);
  }
  .info-box {
    font-family: 'Inter Regular';
    margin-left: 20px;
